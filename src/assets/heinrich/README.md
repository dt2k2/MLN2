...
Background và bộ cutout trạng thái tạo thành motion poster:

| Asset                             | Nội dung                                                                        |
| --------------------------------- | ------------------------------------------------------------------------------- |
| `heinrich-room.webp`              | Căn phòng/xưởng đã xóa Heinrich, giữ nguyên camera, máy móc và ánh sáng.        |
| `01-neutral.png` ... `07-*.png`   | Bảy cutout master có alpha, mỗi ảnh tương ứng một `HeroCondition`.              |
| `01-neutral.webp` ... `07-*.webp` | Bản runtime nén từ PNG, được component preload và crossfade khi đổi trạng thái. |
| `heinrich-cutout.webp`            | Fallback trung tính nếu một cutout trạng thái bị thiếu hoặc tải lỗi.            |
...
dùng randomness trong render. Heinrich có keyframe riêng cho dịch chuyển, nhịp thở, độ
nghiêng và trọng tâm: bình thản ở `neutral`, tiến tới ở `expansion`, cứng giật ở `hardline`,
trĩu xuống ở `market-crisis`, phản ứng nhanh ở `labor-conflict`, rung ngắt quãng ở `rupture`
và tiến gần vững vàng ở `dominant`.
...
- Mỗi cutout có base scale/offset riêng và neo chuyển động ở chân để đổi pose không nhảy vị trí.
...
- [x] Có `heinrich-room.webp`, 7 PNG master, 7 WebP runtime và cutout fallback.
- [x] Bảy trạng thái được preload, crossfade và chuẩn hóa vị trí bằng profile riêng.
...
src/assets/heinrich/
  ...
  heinrich-cutout.webp        # fallback
  01-neutral.png              # master
  01-neutral.webp             # runtime
  02-expansion.png/.webp
  03-hardline.png/.webp
  04-market-crisis.png/.webp
  05-labor-conflict.png/.webp
  06-rupture.png/.webp
  07-dominant.png/.webp
