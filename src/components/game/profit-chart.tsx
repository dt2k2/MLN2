...
function phaseFor(turn: number) {
  if (turn <= 6) return "Di sản hậu 1848";
  if (turn <= 12) return "Nhịp máy mới";
  if (turn <= 18) return "Tập trung & cạnh tranh";
  return "Tín dụng & khủng hoảng";
}
    ...
    <div className="panel-industrial rounded-lg p-3">
      <div className="flex items-center justify-between">
        <div className="text-[10px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">
          {unlocked ? "Xu hướng p′ lý thuyết" : "Xu hướng dôi ra / tổng vốn"}
        </div>
                  ...
                  return (
                    <div className="rounded border border-primary/50 bg-[oklch(0.16_0.008_60)] px-2 py-1.5 font-mono text-[10px] shadow-lg">
                      ...
                      <div className="mt-0.5 text-gold">
                        {unlocked ? "p′ lý thuyết" : "Dôi ra / tổng vốn"} = {p.v.toFixed(1)}%
                      </div>
                    </div>
                  );
                }}
